# -dilema-inovador.html
Dilema do Prisioneiro adaptado como "Dilema do Inovador" (onde você é o Inovador e a Sociedade responde)
O código inclui todas as funcionalidades solicitadas: matriz 2x2 interativa, escolhas (Compartilhar/Guardar), modos manual/aleatório, gráfico em tempo real com Chart.js (via CDN), histórico em tabela, explicação do Equilíbrio de Nash, tema claro/escuro responsivo e cores ouro (#D4AF37) e azul (#1E3A8A). O arquivo funciona offline (exceto Chart.js) e é otimizado para GitHub Pages ou repositórios. O resultado é um jogo educativo em teoria dos jogos, com profundidade técnica para demonstrar conceitos como payoffs, estratégias dominantes e evolução de scores.
index.html
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>
    ...
    <div class="header">
        ...
        <p>Teoria dos Jogos: Escolha entre Compartilhar (cooperação) ou Guardar (defeito) sua inovação. A Sociedade responde. Objetivo: Maximizar seu payoff ao longo de rodadas.</p>
    ...
    <div class="container">
        <div class="matrix">
            ...
            <p><em>Clique em uma célula para escolher sua ação (Compartilhar ou Guardar). A Sociedade responde com base no modo.</em></p>
        </div>
        <div class="controls">
            <h3>Controles</h3>
            <select id="mode" class="mode-select" onchange="setMode()">
                <option value="manual">Modo Manual (Escolha resposta da Sociedade)</option>
                <option value="random">Modo Aleatório (Sociedade escolhe random)</option>
            </select>
            <br>
        ...
        <div class="explanation">
            <h3>Explicação: Equilíbrio de Nash</h3>
            <p>No **Dilema do Prisioneiro** (adaptado aqui para inovação), o Equilíbrio de Nash ocorre quando ambos escolhem "Guardar" (1,1), pois é a estratégia dominante — independentemente da outra parte, maximiza o payoff individual. No entanto, ambos "Compartilharem" (3,3) seria ótimo coletivo, mas instável. Isso ilustra o conflito entre racionalidade individual e coletiva em teoria dos jogos. Payoffs baseados em unidades convencionais: cooperação mútua beneficia todos, mas defeito explora.</p>
            <p><strong>Estratégia de longo prazo:</strong> Em rodadas repetidas (como este jogo), estratégias como "Tit-for-Tat" (copiar a ação anterior da Sociedade) podem emergir para cooperação sustentável, evitando o Nash puro.</p>
        </div>
    </div>
    ...
    <script>
        ...
        function playRound() {
            if (!playerChoice || !societyChoice) {
                alert('Selecione sua escolha e a da Sociedade (ou ative modo aleatório)!');
                return;
            }
        ...
        function clearSelection() {
            ...
            document.querySelectorAll('.payoff').forEach(cell => cell.classList.remove('selected', 'society-choice'));
            if (mode === 'random') {
                societyChoice = null; // Reset para próxima
            }
        }
